<template>
    <div class="container-px-lg post-text-normal">
            ...
            <div>
                <div>
                    <div v-if="$page.props.status.message !== null" class="container-py-inside-md">
                    ...
                    <div class="container-py-inside-md">
                        <Link v-if="$page.props.auth.isDev || $page.props.auth.isAdmin" :href="route('categories.create')" class="post-content font-bold bg-gray-800 px-3 py-2 rounded-md text-white">
                            Add a new Category
                        </Link>
                    </div>
                    <div v-if="categories.length !== 0" class="w-full">
                        <div class="w-full">
                            <div class="w-full font-bold flex flex-wrap py-4 border-b-2 border-gray-800">
                                <div class="category-index-collapse">
                                </div>
                                <div class="category-index-visible">Name</div>
                                <!-- <div class="text-left w-1/7">Description</div> -->
                                <div class="category-index-visible">Posts</div>
                                <div class="category-index-visible">User</div>
                                <div class="category-index-visible">Parent</div>
                                <div class="category-index-invisible">Updated</div>
                            </div>
                        </div>
                        <div class="w-full">
                            <div class="w-full" :key="category.id" v-for="category in categories">
                                <div class="w-full flex flex-wrap container-py-field border-b border-gray-200">
                                    <div class="category-index-collapse  py-2">
                                    ...
                                    <div class="category-index-visible py-2">
                                        <Link :href="route('categories.show', category.id)" class="">
                                            {{category.name}}
                                        </Link>
                                    </div>
                                    <!-- <div class="text-left w-1/7 py-2">{{category.description}}</div> -->
                                    <div class="category-index-visible py-2">{{category.posts_count}}</div>
                                    <div class="category-index-visible py-2">{{category.user.name}}</div>
                                    <div v-if="category.category !== null" class="category-index-visible py-2">
                                        <Link :href="route('categories.show', category.category.id)" class="px-3 py-2 bg-stone-800 rounded-full text-white font-bold">
                                            {{category.category.name}}
                                        </Link>
                                    </div>
                                    <div v-else class="category-index-visible py-2"></div>
                                    <div class="category-index-invisible py-2">{{formatDate(category.updated_at)}}</div>
                                    <div class="flex">
