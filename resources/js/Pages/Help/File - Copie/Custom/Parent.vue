<template>
    <div v-if="objImg">
        <div v-if="objMed">
            <div v-if="objMed.length > 0">
                <div class="w-full font-bold">Current image:</div>
                <div class="flex flex-wrap py-8">
                    <div class="w-full lg:w-1/4 h-56 p-2 md:p-4 border border-gray-200 relative" :key="media.id" v-for="media in objMed">
        ...
        <div v-if="objImg.length > 0" class="flex flex-wrap py-4 md:py-8">
            <div class="w-full font-bold">Selected image:</div>
            <div class="w-full md:w-1/3 lg:w-1/4 h-56 p-2 md:p-4 relative" :key="index" v-for="(file, index) in objImg">
...
export default {
  ...
  methods: {
    ...
    removeFile(file) {
        if (this.collection == 'logo_light') {
            this.object.logo_light = this.object.logo_light.filter(item => item.previewBase64 !== file.previewBase64);
            this.hover = null;
        }
        if (this.collection == 'logo_dark') {
            this.object.logo_dark = this.object.logo_dark.filter(item => item.previewBase64 !== file.previewBase64);
            this.hover = null;
        }
        if (this.collection == 'logo_footer') {
            this.object.logo_footer = this.object.logo_footer.filter(item => item.previewBase64 !== file.previewBase64);
            this.hover = null;
        }
    }
  },
  computed: {
    objImg(){
        return this.collection == 'logo_light' ? this.object.logo_light : this.collection == 'logo_dark' ? this.object.logo_dark : this.collection == 'logo_footer' ? this.object.logo_footer : null;
    },
    objMed(){
        return this.collection == 'logo_light' ? this.object.medias_logo_light : this.collection == 'logo_dark' ? this.object.medias_logo_dark : this.collection == 'logo_footer' ? this.object.medias_logo_footer : null;
    },
  },
  mounted() {
  },
